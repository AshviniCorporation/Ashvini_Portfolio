    ...
    <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
      <div className="max-w-7xl mx-auto sm:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <Link href="/" className="flex gap-2 items-center cursor-pointer group px-6">
...
>
  ...
  >
    <div className="rounded-2xl border border-gray-100 bg-white p-2 shadow-xl">
      ...
      >
        <div className={`${outfit.className} flex flex-col gap-6 px-8 py-8 text-base font-medium`}>
          {primaryLinks.map((link) => (
            <Link key={link.name} href={link.href} className="text-gray-700" onClick={() => setIsOpen(false)}>
              {link.name}
            </Link>
          ))}
          <div className="rounded-2xl bg-gray-50 p-4">
            <p className="text-xs uppercase tracking-[0.3em] text-gray-400">More</p>
            <div className="mt-3 flex flex-col gap-2">
              {dropdownLinks.map((link) => (
                <Link key={link.name} href={link.href} className="text-sm text-gray-600" onClick={() => setIsOpen(false)}>
                  {link.name}
                </Link>
              ))}
            </div>
